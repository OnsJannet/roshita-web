...
export default function Page() {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      ...
      try {
        const response = await fetch("/api/radiologic/getTests", {
          method: "GET",
          headers: {
  ...
  const handleDeleteTest = async (testId: number) => {
    const accessToken = localStorage.getItem("access");
    try {
      const response = await fetch(`/api/radiologic/deleteTest?id=${testId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
          ...
          >
            <Breadcrumb
              items={[
                ...
                {
                  label: language === "ar" ? "الأشعة السينية" : "x-rays",
                  href: "/dashboard/x-rays",
                },
              ]}
              translate={(key) => key}
            />

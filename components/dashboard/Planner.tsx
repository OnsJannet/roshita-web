      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            {/* Doctor Selection (only shown if a hospital is selected) */}
            {selectedHospitalId && (
              <Select
            ...
            {/* Appointment Selection (only shown if a doctor is selected) 
            {selectedDoctorId && (
              <Select
                onValueChange={(value) =>
                  setSelectedAppointmentId(Number(value))
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {hospitals
                    ...
                    ?.appointments.map((appointment, index) => (
                      <SelectItem
                        key={index}
                        value={appointment.id.toString()} // Use appointment ID as the value
                      >
                        {appointment.scheduled_date} - {appointment.start_time}{" "}
                        to {appointment.end_time} (€{appointment.price})
                      </SelectItem>
                    ))}
                </SelectContent>
              </Select>
            ...
            {/* Service Type Input */}
            <Select
              onValueChange={(value) => setServiceType(value)} // Set the service type value
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
